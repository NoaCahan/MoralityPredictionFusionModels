...
class TabNetEncoder(torch.nn.Module):
    ...
    def forward(self, x, prior=None):
        ...
        M_loss /= self.n_steps
        return steps_output, M_loss, out
...
class TabNetPretraining(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            prior = 1 - obf_vars
            steps_out, _, _ = self.encoder(masked_x, prior=prior)
            res = self.decoder(steps_out)
            return res, embedded_x, obf_vars
        else:
            steps_out, _, _ = self.encoder(embedded_x)
            res = self.decoder(steps_out)
...
class TabNetNoEmbeddings(torch.nn.Module):
    ...
    def forward(self, x):
        res = 0
        steps_output, M_loss, emb = self.encoder(x)
...
class EmbeddingGenerator(torch.nn.Module):
    ...
    def __init__(self, input_dim, cat_dims, cat_idxs, cat_emb_dim):
        ...
        #self.continuous_idx = torch.ones(input_dim, dtype=torch.ByteTensor)
        self.continuous_idx = torch.ones(input_dim, dtype=torch.uint8)
...
# Bilinear Attention block
class BANmodel(torch.nn.Module):
    def __init__(self, v_att, b_net, q_prj, c_prj, classifier, counter, alpha):
        super(BANmodel, self).__init__()
        self.op = ''
        self.glimpse = alpha
        self.v_att = v_att
        self.b_net = torch.nn.ModuleList(b_net)
        self.q_prj = torch.nn.ModuleList(q_prj)
    ...
    def forward(self, v, q):
        ...
        for g in range(self.glimpse):
            b_emb[g] = self.b_net[g].forward_with_weights(v_emb, q_emb, att[:,g,:,:])
