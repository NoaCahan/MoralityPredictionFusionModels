...
class BiAttention(nn.Module):
    ...
    def forward_all(self, v, q, v_mask=True):
        ...
        v_num = v.size(1)
        q_num = q.size(1)
        logits = self.logits(v, q).contiguous() # b x g x v x q

        if v_mask:
            mask = (0 == v.abs().sum(2)).unsqueeze(1).unsqueeze(3).expand(logits.size())
            logits.data.masked_fill_(mask.data, -float('inf')).contiguous()

        p = nn.functional.softmax(logits.view(-1, self.glimpse, v_num * q_num), 2).contiguous()
...
class StackedAttention(nn.Module):
    ...
    def forward(self, img_feat, ques_feat, v_mask=True):
        ...
        print(ques_emb_1.shape)
        print(img_emb_1.shape)
